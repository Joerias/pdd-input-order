...
export class Excel {
	...
	public async exportExcel(options: exportExcelType): Promise<void> {
		...
		this.header = header ?? Object.keys(data[0]);
		// 表头行序号
		let headerRowId = 1;
		...
		workbook.xlsx.writeBuffer().then((data) => {
			...
			a.download = today.output("-") + name + ".xlsx";
	...
	private setCells(
		...
		this.header.forEach((item, index) => {
			column.push({
				header: customHeader ? customHeader[index] : item,
				key: item,
				width: style.singleWidth ? style.singleWidth[index] : style?.width || 20,
			});
		});
		...
		this.worksheet!.columns.forEach((column) => {
			column.font = style?.font || { name: "微软雅黑" };
			column.alignment = style?.alignment || { vertical: "middle", horizontal: "left", wrapText: true };
		});
	}
	/**
	 * @description: 添加大标题
